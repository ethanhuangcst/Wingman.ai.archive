  ...
  for (const provider of providers) {
    try {
      await cloudConn.execute(
        `INSERT INTO ai_providers (id, name, base_urls, default_model, requires_auth, auth_header, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          provider.id,
          provider.name,
          JSON.stringify(provider.base_urls),
...
async function restoreChats(localConn, cloudConn) {
  ...
  for (const chat of missingChats) {
    try {
      ...
      console.log(`✅ Restored chat: ${chat.title} (ID: ${chat.id})`);
    } catch (error) {
...
async function restoreChatMessages(localConn, cloudConn) {
  ...
  const missingMessages = localMessages.filter(msg => !cloudMessageIds.includes(msg.id));
