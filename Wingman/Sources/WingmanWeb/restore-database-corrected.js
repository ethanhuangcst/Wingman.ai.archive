    ...
    try {
      await cloudConn.execute(
        ...
        [
          ...
          handleUndefined(chat.name || chat.title || 'Untitled'), // Use name or title or default
          handleUndefined(chat.created_at),
          handleUndefined(chat.updated_at)
        ]
      );
      console.log(`✅ Restored chat: ${chat.name || chat.title || 'Untitled'} (ID: ${chat.id})`);
    } catch (error) {
...
async function verifyRestoration(cloudConn) {
  ...
  // Check chats
  const [chatDetails] = await cloudConn.execute('SELECT id, name, user_id FROM chats');
  console.log('\nChats:');
  chatDetails.forEach(chat => {
    console.log(`  - ${chat.name || 'Untitled'} (ID: ${chat.id}, User: ${chat.user_id})`);
  });
