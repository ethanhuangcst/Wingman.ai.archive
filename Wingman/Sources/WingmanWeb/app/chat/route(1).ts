...
async function getUserAPIInfo(userId: number, selectedProvider?: string): Promise<{ apiProvider: string; apiKey: string }> {
  ...
  try {
    ...
    let query = 'SELECT apiKey, COALESCE(provider_id, apiProvider) as apiProvider FROM ai_connections WHERE user_id = ?';
    let params: any[] = [userId];
    
    if (selectedProvider) {
      // Get connection for the selected provider
      query += ' AND (provider_id = ? OR apiProvider = ?)';
      params.push(selectedProvider, selectedProvider);
    } else {
